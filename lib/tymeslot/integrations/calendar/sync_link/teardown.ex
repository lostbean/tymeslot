defmodule Tymeslot.Integrations.Calendar.SyncLink.Teardown do
  ...
  *other* calendar, held by Google, Outlook or a CalDAV server, and the only
  ...
  # A link may write to a secondary calendar on the target integration, so the
  # integration alone does not locate the placeholder: a delete addressed at the
  # integration's default booking calendar answers 404 for such a one, which is
  # read below as "already gone" — dropping the row and stranding the busy block
  # permanently. `target_calendar_opts/2` is shared with the sync path precisely
  # because that path retries whatever this one fails to withdraw.
  defp withdraw(%CalendarSyncMirrorSchema{} = mirror, link, user_id) do
    opts = CalendarSyncMirrorSchema.target_calendar_opts(mirror, link)
